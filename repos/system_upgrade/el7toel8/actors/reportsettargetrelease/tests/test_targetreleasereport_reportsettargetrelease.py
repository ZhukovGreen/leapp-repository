...
def test_report_target_version(monkeypatch, version):
    monkeypatch.setattr(
        api, 'current_actor', CurrentActorMocked(dst_ver=version)
    )
    ...
    assert (
        SUMMARY_FMT.format(version)
        in reporting.create_report.report_fields['summary']
    )
    assert (
        'is going to be set' in reporting.create_report.report_fields['title']
    )


def test_report_unhandled_release(monkeypatch):
    monkeypatch.setattr(
        api, 'current_actor', CurrentActorMocked(dst_ver='8.1')
    )
    ...
    assert reporting.create_report.called == 1
    assert (
        'is going to be kept' in reporting.create_report.report_fields['title']
    )
