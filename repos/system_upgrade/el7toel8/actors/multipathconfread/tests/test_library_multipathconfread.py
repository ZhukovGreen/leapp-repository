...
default_rhel7_conf = build_config(
    [
        os.path.join(CUR_DIR, 'files/default_rhel7.conf'),
        'directio',
        os.path.join(CUR_DIR, 'files/conf.d'),
        False,
        False,
        False,
        True,
...
all_devs_conf = build_config(
    [
        os.path.join(CUR_DIR, 'files/conf.d/all_devs.conf'),
        None,
        None,
        None,
        None,
...
empty_conf = build_config(
    [
        os.path.join(CUR_DIR, 'files/conf.d/empty.conf'),
        None,
        None,
        None,
        None,
...
default_rhel8_conf = build_config(
    [
        os.path.join(CUR_DIR, 'files/default_rhel8.conf'),
        'tur',
...
all_the_things_conf = build_config(
    [
        os.path.join(CUR_DIR, 'files/all_the_things.conf'),
        'directio',
        os.path.join(CUR_DIR, 'files/conf.d'),
        False,
        False,
...
already_updated_conf = build_config(
    [
        os.path.join(CUR_DIR, 'files/already_updated.conf'),
        None,
        os.path.join(CUR_DIR, 'files/conf.d'),
        None,
        None,
...
ugly1_conf = build_config(
    [
        os.path.join(CUR_DIR, 'files/ugly1.conf'),
        'directio',
        os.path.join(CUR_DIR, 'files/conf.d'),
        False,
        False,
...
ugly2_conf = build_config(
    [
        os.path.join(CUR_DIR, 'files/ugly2.conf'),
        None,
        None,
        None,
        None,
...
just_checker_conf = build_config(
    [
        os.path.join(CUR_DIR, 'files/just_checker.conf'),
        'rdac',
        '/etc/multipath/conf.d',
        True,
        True,
...
just_detect_conf = build_config(
    [
        os.path.join(CUR_DIR, 'files/just_detect.conf'),
        None,
        None,
        None,
        False,
...
just_reassign_conf = build_config(
    [
        os.path.join(CUR_DIR, 'files/just_reassign.conf'),
        None,
        None,
        None,
        None,
...
just_exists_conf = build_config(
    [
        os.path.join(CUR_DIR, 'files/just_exists.conf'),
        None,
        None,
        None,
        None,
...
just_all_devs_conf = build_config(
    [
        os.path.join(CUR_DIR, 'files/just_all_devs.conf'),
        None,
        None,
        None,
        None,
...
def assert_config(config, expected):
    ...
    assert (
        config.ignore_new_boot_devs_exists
        == expected.ignore_new_boot_devs_exists
    )
    assert (
        config.new_bindings_in_boot_exists
        == expected.new_bindings_in_boot_exists
    )
    assert config.unpriv_sgio_exists == expected.unpriv_sgio_exists
    assert (
        config.detect_path_checker_exists
        == expected.detect_path_checker_exists
    )
    assert (
        config.overrides_hwhandler_exists
        == expected.overrides_hwhandler_exists
    )
    assert (
        config.overrides_pg_timeout_exists
        == expected.overrides_pg_timeout_exists
    )
...
def test_config_dir(adjust_cwd):
    ...
    facts = multipathconfread.get_multipath_conf_facts(
        config_file=os.path.join(CUR_DIR, 'files/default_rhel7.conf')
    )
    assert facts
