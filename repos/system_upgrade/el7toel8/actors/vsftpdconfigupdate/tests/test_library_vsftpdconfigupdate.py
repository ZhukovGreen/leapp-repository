...
def test_restoring_default_config():
    ...
    config = VsftpdConfig(
        path=VSFTPD_DEFAULT_CONFIG_PATH,
        tcp_wrappers=False,
...
def test_setting_tcp_wrappers():
    ...
    config = VsftpdConfig(
        path=path, tcp_wrappers=True, strict_ssl_read_eof=False
    )
...
def test_setting_strict_ssl_read_eof():
    ...
    fileops.files[path] = content
    config = VsftpdConfig(
        path=path, tcp_wrappers=None, strict_ssl_read_eof=None
    )
    facts = VsftpdFacts(configs=[config])
