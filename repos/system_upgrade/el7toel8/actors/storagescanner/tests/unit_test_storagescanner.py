...
def test_get_partitions_info(monkeypatch):
    ...
    assert expected == storagescanner._get_partitions_info(
        'tests/files/partitions'
    )

    monkeypatch.setattr(
        storagescanner, '_is_file_readable', is_file_readable_mocked
    )
...
def test_get_fstab_info(monkeypatch):
    ...
    assert expected == storagescanner._get_fstab_info('tests/files/fstab')
    monkeypatch.setattr(
        storagescanner, '_is_file_readable', lambda dummy: False
    )
...
def test_invalid_fstab_info(monkeypatch):
    ...
    storagescanner._get_fstab_info('tests/files/invalid_fstab')
    assert reporting.create_report.called == 1
    assert reporting.create_report.report_fields['severity'] == 'high'
    assert (
        'Problems with parsing data in /etc/fstab'
        in reporting.create_report.report_fields['title']
    )
    assert 'inhibitor' in reporting.create_report.report_fields['flags']
    assert any(
        "The fstab configuration file seems to be invalid" in msg
...
def test_get_mount_info(monkeypatch):
    ...
    assert expected == storagescanner._get_mount_info('tests/files/mounts')
...
def test_get_lsblk_info(monkeypatch):
    def get_cmd_output_mocked(cmd, delim, expected_len):
        return [
            ...
            [
                'rhel_ibm--p8--kvm--03--guest--02-swap',
                '253:1',
                '0',
                '1G',
                '0',
    ...
    monkeypatch.setattr(
        storagescanner, '_get_cmd_output', get_cmd_output_mocked
    )
    expected = [
        LsblkEntry(
            name='vda',
