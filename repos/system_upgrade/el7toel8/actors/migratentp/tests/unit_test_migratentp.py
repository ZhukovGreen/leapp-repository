...
def test_migration(monkeypatch):
    for ntp_services, chrony_services, ignored_lines in [
        ...
        (
            ['ntpd', 'ntpdate', 'ntp-wait'],
            ['chronyd', 'chronyd', 'chrony-wait'],
            1,
        ),
    ]:
        monkeypatch.setattr(reporting, 'create_report', create_report_mocked())
        monkeypatch.setattr(
            migratentp, 'extract_tgz64', extract_tgz64_mocked()
        )
        monkeypatch.setattr(
            migratentp, 'enable_service', enable_service_mocked()
        )
        monkeypatch.setattr(migratentp, 'write_file', write_file_mocked())
        monkeypatch.setattr(
            migratentp, 'ntp2chrony', ntp2chrony_mocked(ignored_lines)
        )
        ...
        if ntp_services:
            ...
            if ignored_lines > 0:
                assert (
                    'configuration partially migrated to chrony'
                    in reporting.create_report.report_fields['title']
                )
            else:
                assert (
                    'configuration migrated to chrony'
                    in reporting.create_report.report_fields['title']
                )
            ...
            assert migratentp.write_file.called == (
                0 if 'ntpd' in ntp_services else 1
            )
            if migratentp.write_file.called:
                assert migratentp.write_file.name == '/etc/ntp.conf.nosources'
                assert (
                    'without ntp configuration'
                    in migratentp.write_file.content
                )
            assert migratentp.ntp2chrony.called == 1
            assert migratentp.ntp2chrony.args == (
                '/',
                '/etc/ntp.conf'
                if 'ntpd' in ntp_services
                else '/etc/ntp.conf.nosources',
