...
class MigrateBrltty(Actor):
    ...
    def process(self):
        for decision in self.consume(BrlttyMigrationDecision):
            report_summary = ''
            migratebrltty.migrate_file(
                decision.migrate_file,
                decision.migrate_bt,
                decision.migrate_espeak,
            )
            if decision.migrate_bt:
                ...
                report_summary = report_summary.format(
                    ', '.join(decision.migrate_file)
                )
            if decision.migrate_espeak:
                if report_summary:
                    report_summary += '\n'
                report_summary += (
                    'eSpeak speech driver was switched to eSpeak-NG in {}'
                )
                report_summary = report_summary.format(
                    ', '.join(decision.migrate_file)
                )
