...
def test_rewrite_spamd_option_no_assignment():
    content = '# foo\nbar=foobar\n'
    ops = [
        spamassassinconfigupdate_spamd._drop_ssl_version,
        spamassassinconfigupdate_spamd._drop_daemonize_option,
    ]
    rewritten = spamassassinconfigupdate_spamd._rewrite_spamd_option(
        content, ops
    )
...
def test_rewrite_spamd_option_last_assignment_takes_effect():
    ...
    content = (
        'SPAMDOPTIONS="--ssl-version tlsv1 --ssl"\n'
...
def test_rewrite_spamd_config():
    ...
    content = (
        '# Options passed to spamd\n'
        'SPAMDOPTIONS="-c -d -m5 --ssl -H --ssl-version tlsv1"\n'
    )

    rewritten = spamassassinconfigupdate_spamd._rewrite_spamd_config(
        facts, content
    )

    assert (
        rewritten == '# Options passed to spamd\n'
...
def test_rewrite_spamd_config_service_overriden():
    ...
    facts = SpamassassinFacts(
        spamd_ssl_version='sslv3', service_overriden=True
    )
    content = (
        '# Options to spamd\n'
...
def test_migrate_spamd_config():
    facts = SpamassassinFacts(
        service_overriden=False, spamd_ssl_version='tlsv1'
    )
    fileops = MockFileOperations()
    content = (
        '# foo\n'
        + SYSCONFIG_VARIABLE
        + '="-c --ssl-version tlsv1 -hdx"\n'
        + '# bar \n'
    )
    fileops.files[SYSCONFIG_SPAMASSASSIN] = content
    backup_func = MockBackup()

    spamassassinconfigupdate_spamd.migrate_spamd_config(
        facts, fileops, backup_func
    )
