...
def test_add_boot_entry_non_s390x(monkeypatch):
    ...
    monkeypatch.setattr(
        addupgradebootentry, 'get_boot_file_paths', get_boot_file_paths_mocked
    )
    monkeypatch.setenv('LEAPP_DEBUG', '1')
    monkeypatch.setattr(addupgradebootentry, 'run', run_mocked())
    monkeypatch.setattr(
        api, 'current_actor', CurrentActorMocked(architecture.ARCH_X86_64)
    )
...
def test_add_boot_entry_s390x(monkeypatch):
    ...
    monkeypatch.setattr(addupgradebootentry, 'run', run_mocked())
    monkeypatch.setattr(
        api, 'current_actor', CurrentActorMocked(architecture.ARCH_S390X)
    )
...
def test_get_boot_file_paths(monkeypatch):
    ...
    monkeypatch.setattr(
        'leapp.libraries.stdlib.api.consume', consume_message_mocked
    )
    ...
    monkeypatch.setattr(
        'leapp.libraries.stdlib.api.consume', consume_no_message_mocked
    )
...
def test_fix_grub_config_error(monkeypatch):
    monkeypatch.setattr(
        addupgradebootentry, 'write_to_file', write_to_file_mocked()
    )
    addupgradebootentry.fix_grub_config_error('files/grub_test.wrong')
