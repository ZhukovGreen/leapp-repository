...
_CUSTOM_REPOS = [
    CustomTargetRepository(
        repoid='repo1', name='repo1name', baseurl='repo1url', enabled=True
    ),
    CustomTargetRepository(
        repoid='repo2', name='repo2name', baseurl='repo2url', enabled=False
    ),
    CustomTargetRepository(
        repoid='repo3', name='repo3name', baseurl=None, enabled=True
    ),
    CustomTargetRepository(
        repoid='repo4', name='repo4name', baseurl=None, enabled=True
    ),
]

_TARGET_REPOS_CUSTOM = TargetRepositories(
    rhel_repos=_RHEL_REPOS, custom_repos=_CUSTOM_REPOS
)
_TARGET_REPOS_NO_CUSTOM = TargetRepositories(rhel_repos=_RHEL_REPOS)
_CUSTOM_TARGET_REPOFILE = CustomTargetRepositoryFile(
    file='/etc/leapp/files/leapp_upgrade_repositories.repo'
)
...
def test_checktargetrepos_no_rhsm(
    monkeypatch, enable_repos, custom_target_repos, custom_target_repofile
):
    mocked_consume = MockedConsume(
        _TARGET_REPOS_CUSTOM
        if custom_target_repos
    ...
    if not custom_target_repos:
        ...
        not enable_repos and custom_target_repos and not custom_target_repofile
    ):
        assert reporting.create_report.called == 1
