...
def destructive_selinux_env():
    tests_dir = os.path.join(
        os.path.realpath(__file__).rsplit(os.path.sep, 2)[0], testmoduledir
    )
    ...
    for command in SEMANAGE_COMMANDS[:-1]:
        _run_cmd(
            ['semanage', command[0], '-d'] + command[1:],
            'Error removing selinux customizations after testing',
        )

    for priority, module in reversed(
        TEST_MODULES + [['400', 'permissive_abrt_t']]
    ):
        _run_cmd(
