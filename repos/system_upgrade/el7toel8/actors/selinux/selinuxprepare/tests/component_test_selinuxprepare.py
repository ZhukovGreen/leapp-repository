...
def semodule_lfull_initial():
    yield _run_cmd(
        ['semodule', '-lfull'], logmsg='Error listing SELinux customizations'
    )
...
def semanage_export_initial():
    yield _run_cmd(
        ['semanage', 'export'], logmsg='Error listing SELinux customizations'
    )
...
def destructive_selinux_env():
    for priority, module in TEST_MODULES:
        tests_dir = os.path.join(
            os.getenv('PYTEST_CURRENT_TEST').rsplit(os.path.sep, 2)[0],
            testmoduledir,
        )
        _run_cmd(
            [
                ...
                priority,
                '-i',
                os.path.join(tests_dir, module + '.cil'),
            ],
    ...
    for command in SEMANAGE_COMMANDS:
        _run_cmd(
            ['semanage', command[0], '-a'] + command[1:],
            logmsg='Error applying selinux customizations',
        )
...
def test_SELinuxPrepare(
    current_actor_context,
    semodule_lfull_initial,
    semanage_export_initial,
    ...
    semodule_res = _run_cmd(
        ['semodule', '-lfull'], 'Error listing SELinux modules'
    )
    assert semodule_lfull_initial == semodule_res
    semanage_res = _run_cmd(
        ['semanage', 'export'], 'Error listing SELinux customizations'
    )
