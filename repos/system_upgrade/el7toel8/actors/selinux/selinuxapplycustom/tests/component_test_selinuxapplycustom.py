...
def destructive_selinux_teardown():
    ...
    for command in SEMANAGE_COMMANDS[1:]:
        _run_cmd(
            ['semanage', command[0], '-d']
            + [x.strip('"\'') for x in command[1:]],
            'Failed to remove SELinux customizations after testing',
        )

    _run_cmd(
        ['semanage', SEMANAGE_COMMANDS[0][0], '-d'] + SEMANAGE_COMMANDS[0][1:],
        'Failed to remove SELinux customizations after testing',
    )
...
def test_SELinuxApplyCustom(
    current_actor_context, destructive_selinux_teardown
):
    ...
    semodule_lfull = _run_cmd(
        ['semodule', '-lfull'], 'Error listing selinux modules'
    )
    semanage_export = _run_cmd(
