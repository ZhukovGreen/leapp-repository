...
def test_setrelease(monkeypatch):
    ...
    monkeypatch.setattr(
        api, 'current_actor', CurrentActorMocked(dst_ver='8.0')
    )
...
def test_setrelease_submgr_throwing_error(monkeypatch):
    ...
    if sys.version_info.major < 3:
        monkeypatch.setattr(
            rhsm,
            'set_release',
            rhsm.set_release.func_closure[0]
            .cell_contents.func_closure[0]
            .cell_contents,
        )
    else:
        monkeypatch.setattr(
            rhsm, 'set_release', rhsm.set_release.__wrapped__.__wrapped__
        )
...
def test_setrelease_skip_rhsm(monkeypatch, product):
    ...
    monkeypatch.setattr(
        api, 'current_actor', CurrentActorMocked(envars={'LEAPP_NO_RHSM': '1'})
    )
...
def test_get_unique_repoids(monkeypatch):
    ...
    assert enablerhsmreposonrhel8.get_repos_to_enable() == {
        'some-repo',
        'another-repo',
    }
...
def test_get_submgr_cmd():
    assert enablerhsmreposonrhel8.get_submgr_cmd({'some-repo'}) == [
...
def test_running_submgr_ok(monkeypatch):
    ...
    monkeypatch.setattr(
        enablerhsmreposonrhel8, 'get_repos_to_enable', lambda: {'some-repo'}
    )
...
def test_running_submgr_fail(monkeypatch):
    ...
    monkeypatch.setattr(
        enablerhsmreposonrhel8, 'run', run_mocked(raise_err=True)
    )
    monkeypatch.setattr(api, 'current_logger', logger_mocked())
    enablerhsmreposonrhel8.enable_rhsm_repos()
    assert enablerhsmreposonrhel8.run.called
